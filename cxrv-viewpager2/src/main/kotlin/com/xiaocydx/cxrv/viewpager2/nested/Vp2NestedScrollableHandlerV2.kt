 ...
 * @date 2022/7/8
 */
class Vp2NestedScrollableHandlerV2 {
     ...
     */
    fun handleInterceptTouchEvent(child: View, childOrientation: Int, e: MotionEvent) {
        ...
        when (e.action) {
            ...
            MotionEvent.ACTION_MOVE -> {
                ...
                val isChildHorizontal = childOrientation == ORIENTATION_HORIZONTAL
                val xDiff = dx.absoluteValue
                val yDiff = dy.absoluteValue
                if (xDiff > childTouchSlop || yDiff > childTouchSlop) {
                    val disallowIntercept = if (isVp2Horizontal) {
                        if (child.canScrollHorizontally(dx.toDirection())) {
                            ...
                            dx.absoluteValue > 2 * dy.absoluteValue
                        } else {
                            !isChildHorizontal && 2 * dy.absoluteValue > dx.absoluteValue
                        }
                    } else {
                        if (child.canScrollVertically(dy.toDirection())) {
                            // child能和ViewPager2平行滚动，不允许ViewPager2拦截触摸事件
                            dy.absoluteValue > 2 * dx.absoluteValue
                        } else {
                            isChildHorizontal && 2 * dx.absoluteValue > dy.absoluteValue
                        }
                    }
                    child.parent.requestDisallowInterceptTouchEvent(disallowIntercept)
                    isNestedScrollableHandled = true
                }
            }
        }
