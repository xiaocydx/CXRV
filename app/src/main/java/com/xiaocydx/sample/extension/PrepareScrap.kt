...
/**
 * // TODO: 2022/11/11 目前仅支持预创建单类型，仍需改善。
 *
 * 往[RecycledViewPool]预创建[prepareCount]个ViewHolder
 *
 * 预创建流程会尽量在主线程执行RecyclerView布局流程之前停止，避免创建冗余的ViewHolder。
 *
 * @param prepareCount      预创建的ViewHolder数量
 * @param prepareAdapter    调用[Adapter.createViewHolder]创建ViewHolder
 * @param prepareDispatcher 用于预创建流程的协程调度器，默认为[DefaultIoDispatcher]
 * @param prepareViewType   预创建的ViewType类型
 *
 * @return 收集返回的`Flow<ViewHolder>`，才开始执行预创建，
...
fun RecyclerView.prepareScrap(
    ...
    coroutineScope {
        ...
        val deadlineJob = launch(start = UNDISPATCHED) {
            // 视图树首帧Vsync时间或者下一个Vsync预测时间，作为预创建的截止时间
            deadlineNs.set(awaitDeadlineNs(prepareAdapter))
        }
...
@MainThread
private suspend fun RecyclerView.awaitDeadlineNs(
    adapter: Adapter<*>
): Long = suspendCancellableCoroutine { cont ->
    DeadlineNsObserver(this, adapter, cont).attach()
}
...
private class DeadlineNsObserver(
    ...
    private fun tryResume() {
        if (isResumed) return
        isResumed = true
        val latestFrameVsyncNs = TimeUnit.MILLISECONDS.toNanos(rv.drawingTime)
        if (latestFrameVsyncNs <= 0) {
            // 视图树的首帧绘制未完成，将首帧Vsync时间作为截止时间
            Choreographer.getInstance().postFrameCallback(::resume)
        } else {
            // 视图树的首帧绘制已完成，将下一个Vsync预测时间作为截止时间
            resume(calculateDeadlineNs(latestFrameVsyncNs))
        }
    }
    ...
    private fun calculateDeadlineNs(latestFrameVsyncNs: Long): Long {
        val frameIntervalNs = calculateFrameIntervalNs()
        val frameInterval = (System.nanoTime() - latestFrameVsyncNs) / frameIntervalNs
        // 下一个Vsync预测时间
        return latestFrameVsyncNs + (frameInterval * frameIntervalNs) + frameIntervalNs
    }

    private fun calculateFrameIntervalNs(): Long {
        // TODO: 2022/4/11 减少计算frameIntervalNs的次数
        val display = ViewCompat.getDisplay(rv)
