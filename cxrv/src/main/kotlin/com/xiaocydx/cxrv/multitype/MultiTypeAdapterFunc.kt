 ...
 */
inline fun listAdapter(block: MultiTypeScope<Any>.() -> Unit) = listAdapter<Any>(block)
...
inline fun <T : Any> listAdapter(block: MultiTypeScope<T>.() -> Unit): ListAdapter<T, *> {
    return MultiTypeScope<T>().apply(block).complete()
}
...
inline fun <reified T : Any> ViewTypeDelegate<T, *>.toListAdapter(): ListAdapter<T, *> {
    return MultiTypeAdapter(SingletonMultiType(T::class.java, this))
}