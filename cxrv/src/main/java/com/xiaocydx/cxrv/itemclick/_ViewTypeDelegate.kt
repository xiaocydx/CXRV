...
import com.xiaocydx.cxrv.list.DisposableWrapper
...
inline fun <ITEM : Any, VH : ViewHolder> ViewTypeDelegate<out ITEM, out VH>.doOnItemClick(
    ...
    doOnAttach {
        adapter.doOnItemClick(
            ...
            action = { holder, item ->
                holder.ifViewTypeSame(viewType)?.let { action(it as VH, item as ITEM) }
            }
        ).also(wrapper::attach)
...
inline fun <ITEM : Any, VH : ViewHolder> ViewTypeDelegate<out ITEM, out VH>.doOnLongItemClick(
    ...
    doOnAttach {
        adapter.doOnLongItemClick(
            ...
            action = { holder, item ->
                holder.ifViewTypeSame(viewType)?.let { action(it as VH, item as ITEM) } ?: false
            }
        ).also(wrapper::attach)
    }.also(wrapper::attachIfEmpty)
}
